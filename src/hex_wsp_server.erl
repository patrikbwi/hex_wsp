...
handle_call({add_event,Flags,Signal,Cb}, _From, State) ->
    case wsp_server:subscribe(Flags) of
	{ok,Ref} ->
...
handle_call({del_event,Ref}, _From, State) ->
	...
	{value,_Sub, Subs} ->
	    Res = wsp_server:unsubscribe(Ref),
...
%% @end
%%--------------------------------------------------------------------
handle_info({wsp_event,Ref,EventData}, State) ->
    lager:debug("wsp_event: ref=~p, event=~p", [Ref, EventData]),
